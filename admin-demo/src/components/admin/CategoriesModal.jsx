...
const CategoriesModal = ({ category: editingCategory, onSave, onClose, isOpen, fetchData }) => {
...
const generateSlug = (name) => {
  ...
  // remove unwanted characters (safe regex, no repetition issue)
  str = str.replace(/[^\w\s-]/g, '');

  // replace spaces, underscores, dashes with single dash (NO regex)
  let result = '';
  let prevDash = false;
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-2xl rounded-[40px] min-h-400px overflow-auto " title={isEdit ? "Edit Category" : "New Category"} size="lg">
