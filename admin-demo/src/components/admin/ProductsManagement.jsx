...
const ProductsManagement = ({
   ...
   const getBulkActions = (selection = []) => {
    const allDeleted = selection.every(p => p.is_soft_deleted);
    const allActive = selection.every(p => !p.is_soft_deleted);
    ...
    if (!allDeleted) {
        actions.push({
            ...
            severity: "danger",
            handler: async (selection) => {
                const confirmed = await confirm(`Archive ${selection.length} product${selection.length > 1 ? 's' : ''}?`);
                if (confirmed) {
    ...
    if (!allActive) {
        actions.push({
            ...
            icon: ArrowPathIcon,
            handler: async (selection) => {
                const confirmed = await confirm(`Restore ${selection.length} product${selection.length > 1 ? 's' : ''}?`);
                if (confirmed) {
...
const bulkActions = getBulkActions(selection);
    return (
                            ...
                            body: (p) => (
                                <div className="flex gap-2">
                                    ...
                                    {p.is_soft_deleted ? (
                                        <button
                                            onClick={async () => {
                                                const confirmed = await confirm(`Restore product "${p.title}"?`);
                                                if (confirmed) {
                                                    onRestore(p.product_id);
                                                }
                                            }}
                                        ...
                                        <button
                                            onClick={async () => {
                                                const confirmed = await confirm(`Archive product "${p.title}"?`);
                                                if (confirmed) {
