...
const useDataTable = ({ 
    ...
    //common function of fetch the API
    const fetchData = useCallback(async () => {
        if (!fetchFn) return; //if no API 
        
        try {
            setLoading(true);
            const res = await fetchFn(lazyParams); //call API from table 
    ...
    //debounce funtion for perevent call API on every letter of search
   ...
   // clear the debounce component when unmount
    useEffect(() => {
        return () => debouncedSearch.cancel();
    }, [debouncedSearch]);


    //search function with call the debounced search 
    ...
    //filter funtion to pervent the flase value from the filter 
    ...
    //lazy load for primereact data table with sanitizefilter 
