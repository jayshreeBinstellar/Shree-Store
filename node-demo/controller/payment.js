...
const processSuccessfulPayment = async (session) => {
  ...
  try {
    ...
    //paymentIntentId is unique for each successful payment. If we find an order with same paymentIntentId, it means this payment has already been processed and we should not create duplicate orders.
    ...
    //parseFloat is function that convert string to number wiht decimal
    // we need to convert the string to number because in the metadata we store all values as string, but in the database we need to store them as number. If we don't convert them, they will be stored as string in the database and that will cause problems when we try to do calculations with those values later.
    const orderValues = [
      userId,
      parseFloat(subtotal),
...
//stripe webhook (what is webhook? answer: webhook is a way of stripe to notify our server is event happen in the sripe side. for example, when payment is successful, stripe will send a request to our webhook endpoint to notify us that payment is successful. we can then process the order in the webhook handler.)
exports.webhook = async (req, res) => {
  const sig = req.headers["stripe-signature"];
  let event;

  try {
    event = stripe.webhooks.constructEvent( /* raw body */
      req.body,
      sig,
